...
class MessageViewController : SLKTextViewController {
    ...
    override func didPressRightButton(sender: AnyObject!) {
        ...
        messageEntity.save { (response) -> Void in
            ...
            if let createdMessageEntity = response.entity {

                createdMessageEntity.connect("messageCreator", toEntity: Usergrid.currentUser!) { (response) in
                    if let errorDescription = response.errorDescription {
                        print("Connecting message to user error: \(errorDescription)")
                    }
                }
            }
        }
    ...
    func messageCellForRowAtIndexPath(indexPath:NSIndexPath) -> MessageTableViewCell {
        ...
        if let username = messageEntity[MessageViewController.MESSAGE_ENTITY_CREATOR] as? String {
            cell.titleLabel.text = username;
            messageEntity.getConnections(.Out, relationship: "messageCreator") { (response) in
                if let messageCreator = response.user {
                    cell.titleLabel.text = messageCreator.username
                    if let profileImage = messageCreator["picture"] as? String {
